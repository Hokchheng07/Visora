...
Popular <span className="text-primary">Template<span className="text-secondary">s</span></span>
        ...
        >
          {templates.map(({ image, title }, index) => (
            <motion.article
              key={`${title}-${index}`}
              className="relative rounded-[22px] border border-[#e6ccff] bg-white shadow-[0_10px_22px_rgba(112,90,224,.12)]"
              variants={fadeInUp}
              whileHover={{ y: -6 }}
            ...
            >
              <div className="relative z-10 overflow-hidden rounded-[22px]">
                ...
                <div className="flex items-center gap-2 px-5 pb-5 pt-4 text-[12px] text-[#666]">
