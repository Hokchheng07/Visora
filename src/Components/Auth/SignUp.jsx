    ...
    <main className="min-h-dvh bg-white font-sans dark:bg-black lg:h-dvh">
      <section className="min-h-0 overflow-y-auto overflow-x-hidden px-5 py-8 sm:px-10 sm:py-12 lg:flex lg:h-full lg:items-start lg:justify-center lg:px-16 lg:py-16 xl:px-24 xl:py-20">
        <div className="mx-auto w-full max-w-[480px] lg:my-auto lg:max-w-[650px]">
          ...
          <header className="relative mb-6 max-w-[560px] lg:mb-8">
            ...
            <p className="mt-4 max-w-[500px] text-base leading-6 text-gray-500 sm:text-lg lg:mt-6 lg:text-xl lg:leading-7 dark:text-[#bcbccd]">Join Visora and start designing stunning event backdrops in minutes</p>
          </header>
          ...
          <form onSubmit={handleSubmit(handleRegisterSubmit)} noValidate>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 sm:gap-x-8">
              {fields.map(({ name, label, placeholder, icon: Icon, full, type = "text" }) => (
                <label key={name} className={`block ${full ? "col-span-2" : ""}`}>
                  <span className="mb-1 block text-sm font-semibold text-gray-700 sm:text-base lg:mb-1.5 lg:text-lg dark:text-gray-200">{label} <span className="text-red-600">*</span></span>
                  <span className="relative block">
                    ...
                    <input
                      ...
                      className={`h-11 w-full rounded-lg border bg-white dark:bg-[#1a1a28] px-4 text-sm text-gray-700 outline-none dark:text-gray-100 transition focus:border-primary focus:ring-2 focus:ring-primary/20 sm:text-base lg:h-12 ${full || type === "password" ? "pl-12 lg:pl-16" : ""} ${type === "password" ? "pr-12" : ""} ${errors[name] ? "border-red-500" : "border-gray-300 dark:border-white/15"}`}
                      {...register(name)}
                    />
                    {type === "password" && (
                      <button
            ...
            <label className="mt-4 flex items-start gap-2 text-sm leading-5 text-gray-600 lg:mt-5 dark:text-[#bcbccd]">
            ...
            <button type="submit" className="hero-cta hero-cta-primary mt-4 h-11 w-full max-w-none gap-3 text-base lg:mt-5 lg:h-12 lg:text-xl">
              <span>Sign Up</span><ArrowRight className="size-5 shrink-0 lg:size-6" strokeWidth={2.25} aria-hidden="true" />
            </button>

            <div className="my-4 flex items-center gap-3 text-base text-gray-400 lg:my-5 lg:gap-4 lg:text-xl"><span className="h-px flex-1 bg-gray-300 dark:bg-white/15" />or Sign up with<span className="h-px flex-1 bg-gray-300 dark:bg-white/15" /></div>
            <div className="grid gap-2.5">
              <button type="button" className="flex h-11 items-center justify-center rounded-lg border border-gray-300 text-base text-gray-800 transition hover:bg-gray-50 lg:h-12 lg:text-lg dark:text-gray-100 dark:border-white/15 dark:hover:bg-white/5"><span className="grid w-[19rem] max-w-[calc(100%-2rem)] grid-cols-[1.5rem_1fr] items-center gap-3 text-left"><img src={googleIcon} alt="" className="h-5 w-5 justify-self-center lg:h-6 lg:w-6" /><span>Continue with Google</span></span></button>
            ...
            <p className="mt-4 text-center text-gray-400">Already have an account? <Link to="/auth/login" className="font-medium text-primary hover:underline">Log in</Link></p>
          </form>
        </div>
      </section>
    </main>
