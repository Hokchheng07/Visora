  ...
  return (
    <div className="about-page">
      {/* Drifting dust: the page's background texture, swirling towards the
          pointer. It reaches up by the navbar's height, because the navbar's
          wave cutout and the blob's translucency both expose the strip above
          this page's own box — dust stopping at the box top would read as a
          flat, dot-less band under the header. */}
      <CosmicDustField particleCount={220} className="cosmic-dust-under-nav" />
      ...
      <div className="about-page-art" aria-hidden="true">
        {/* Plugs the same strip with a flat fill in .about-page's own
            ...
            under the navbar otherwise. Sits below the blob/Angkor art in
            paint order (first child, no z-index of its own), so it never
            covers them. */}
