...
import { effectiveLocked, layerLabel, stepLayers, selectedGroup } from "../model/layerModel.js";
...
export default function EditorInspector({ docked, onClose }) {
  ...
  if (selection.length > 1 || group) {
    ...
    if (element.type === "timer") {
      header = <Header icon={TimerIcon} title={element.name || (element.timer?.mode === "STOPWATCH" ? "Stopwatch" : "Timer")} onClose={close}>{actions}</Header>;
