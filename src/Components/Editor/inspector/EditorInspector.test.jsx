...
describe("EditorInspector", () => {
  ...
  it("a stopwatch shows only its own settings; colours for Start, Stop and Reset live here", () => {
    const { element, editor } = setup([timerInserted("STOPWATCH")]);
    expect(screen.getByRole("heading", { name: "Stopwatch", level: 2 })).toBeTruthy();
    const titles = [...document.querySelectorAll(".editor-inspector-group h3")].map((heading) => heading.textContent);
    expect(titles).not.toContain("Countdown");
    expect(titles).not.toContain("When time ends");
    expect(titles).toContain("Button colours");
    expect(screen.queryByText("Pause / Resume")).toBe(null);
    // "Reset" is both a control toggle and a colour row now; the toggle is the one with a switch.
    expect(screen.getAllByText("Reset").some((node) => node.closest("label")?.querySelector("input[type='checkbox']"))).toBe(true);
    expect(screen.queryByLabelText("Pause / Resume button hex")).toBe(null);
    const stop = screen.getByLabelText("Stop button hex");
    fireEvent.focus(stop);
    fireEvent.change(stop, { target: { value: "112233" } });
