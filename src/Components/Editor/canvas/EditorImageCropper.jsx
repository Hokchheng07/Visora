...
import { DEFAULT_CROP, MAX_ZOOM, normalizeCrop, panCrop } from "../model/imageCrop.js";
 ...
 */
export default function EditorImageCropper({ element, pageId, scale }) {
  ...
  return (
    <div className="editor-crop" data-crop-for={element.id}
      ...
      onPointerMove={(event) => {
        ...
        if (!active) return;
        move(panCrop(active.crop, (event.clientX - active.x) * unit, (event.clientY - active.y) * unit, element.w, element.h));
      }}
      ...
      <div className="editor-crop-bar" onPointerDown={(event) => event.stopPropagation()}>
