...
function harness({ animations = [row("in"), row("out", { kind: "exit", trigger: "click" })], transition = { durationMs: 700, delayMs: 0 }, morph = false } = {}) {
  const frames = {}, transitions = [], drivers = []; let nextPages = 0;
  const controller = createPlaybackController({ page: { elements: [{ id: "a" }], animations }, transition, morph,
