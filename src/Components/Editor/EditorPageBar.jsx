...
import { pageBackgroundChanged } from "../redux/editorSlice.js";
import { pageLabel } from "./layerModel.js";
import { SwatchButton } from "./EditorControls.jsx";
...
export default function EditorPageBar({ onAnimate }) {
  ...
  return <PageProperties page={pages[currentPage]} label={pageLabel(pages[currentPage], currentPage)} busy={!!gesture} dispatch={dispatch} onAnimate={onAnimate} />;
}
...
function PageProperties({ page, label, busy, dispatch, onAnimate }) {
  ...
  return (
    <div className="editor-shape-tools" role="group" aria-label="Page settings"
      ...
      <div className="editor-toolbar-group">
        <button type="button" aria-label="Page animation" disabled={busy} onClick={onAnimate}><Sparkles size={16} aria-hidden="true" /><span className="editor-ctl-text">Animate</span></button>
      </div>
    </div>
  );
}
